...
The published evaluation is on ARC-AGI-1. **Evidence type: developer-reported; the headline score is partially independently audited; the internal LOW/MEDIUM/HIGH ablation (Table 5) is developer-reported only, not independently reproduced.**
...
5. **Test set size**: 100 mazes is sufficient to show the scaling trend clearly but too small to make strong statistical claims about exact solve rates — the 88% figure is ±~3% at 95% confidence. Do not over-interpret the specific numbers; interpret the shape.
...
Pathway's BDH-CQ results on ARC-AGI-1 show the same structure at a different scale, on a harder task, at developer-reported (partially audited) confidence. The toy and the frontier are doing structurally the same thing. This artifact exists to make that structure visible.
